    ...
    stage('Build container') {
        steps {
            container('buildah') {
                ...
                sh 'containerid=$(buildah from --cap-add=CAP_SETUID --cap-add=CAP_SETGID --isolation chroot --security-opt seccomp=unconfined --security-opt apparmor=unconfined --userns host nexus-docker.apps.afs-demo.openshiftpoc.us/opensource/nodejs:latest)'
