    ...
    stage('Build container') {
        steps {
            container('podman') {
                ...
                sh 'ls'
                sh 'podman build --cap-add SYS_ADMIN --cgroup-manager=cgroupfs --storage-driver=vfs --net=host -t nexus-docker.apps.afs-demo.openshiftpoc.us/apps/react-ant-app /home/jenkins/agent/workspace/react-pipeline'
                sh 'podman images'
            }
        }
    }
