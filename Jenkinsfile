    ...
    stage('Build container') {
        steps {
            container('podman') {
                ...
                sh 'ls'
                sh 'podman build --userns=keep-id --isolation chroot --cap-add CAP_SYS_ADMIN --cgroup-manager=cgroupfs --device /dev/fuse --storage-driver=overlay --storage-opt "overlay.mount_program=/usr/bin/fuse-overlayfs" --net=host --security-opt seccomp=unconfined --security-opt label=disabled -t podman-react /home/jenkins/agent/workspace/react-pipeline'
