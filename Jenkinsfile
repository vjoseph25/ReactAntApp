    ...
    stage('Build container') {
        steps {
            container('podman') {
                ...
                sh 'cat /etc/passwd'
                sh 'podman build --cap-add SYS_ADMIN --device /dev/fuse -t nexus-docker.apps.afs-demo.openshiftpoc.us/apps/react-ant-app /home/jenkins/agent/workspace/react-pipeline'
                sh 'podman images'
            }
        }
    }
