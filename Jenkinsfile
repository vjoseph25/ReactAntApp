    ...
    stage('Build container') {
        steps {
            container('podman') {
                ...
                sh 'ls'
                sh 'podman build --cap-add CAP_SYS_ADMIN --cgroup-manager=cgroupfs --device /dev/fuse --storage-driver=overlay --net=host --security-opt seccomp=unconfined --security-opt label=disabled -t nexus-docker.apps.afs-demo.openshiftpoc.us/apps/react-ant-app /home/jenkins/agent/workspace/react-pipeline'
                sh 'podman images'
            }
        }
    }
