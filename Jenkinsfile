    ...
    stage('Build container') {
        steps {
            container('buildah') {
                ...
                sh 'containerid=$(buildah from --cap-add=CAP_SETUID --cap-add=CAP_SETGID --isolation chroot --userns host nexus-docker.apps.afs-demo.openshiftpoc.us/opensource/nodejs:latest)'
